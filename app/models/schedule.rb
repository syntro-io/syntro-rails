class Schedule < ActiveRecord::Base
  ...
  def self.jmeter(result)
    ...
    if result.test_case.uploads.first.nil?
      ...
      if jmeter_params[:certificate].nil? || (jmeter_params[:certificate] == '') || (jmeter_params[:certificate] == 'none')
  ...
  def self.sikuli(result)
    ...
    if result.test_case.uploads.first.nil?
      ...
      # Log into host machine
      if sikuli_params[:certificate].nil? || (sikuli_params[:certificate] == '') || (sikuli_params[:certificate] == 'none')
        LOGGER.info 'Using the password method.'
  ...
  def self.analyze_results(result)
    ...
    result.uploads.each do |upload|
      ...
      next if upload.upload_file_name == 'jmeter.log'
      ...
      doc.xpath(xpath_string).each do |element|
        ...
        failed = true if sample[:success] == 'false'
        samples << sample
      end
      ...
      results.each do |_key, thread|
        thread.each do |_key, page|
      ...
      page_results.each do |_key, page|
  ...
  def self.copy_file_to_result(result, host, filename)
    ...
    filename = host[:working_dir] + filename unless filename =~ /^\//

    # Copy the file using SCP to the rails tmp/ folder
    # Use the password login
    if host[:certificate].nil? || (host[:certificate] == '') || (host[:certificate] == 'none')

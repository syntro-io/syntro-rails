class TestPlansController < ApplicationController
  ...
  def edit
    ...
    if (Setting.value('Allow Test Plan Edit After Assignment') == true) || (Assignment.where(test_plan_id: @test_plan.id).count < 1)
      # @test_cases = TestCase.find(:all)
      @products = current_user.products.order('name')
  ...
  def generate_rtf(test_plan)
    ...
    test_plan.test_cases.order('case_order').each do |test_case|
      ...
      next unless test_case.steps.count > 0

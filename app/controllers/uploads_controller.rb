class UploadsController < ApplicationController
  ...
  def download
    ...
    head(:not_found) && return if (upload = Upload.find_by_id(params[:id])).nil?
    head(:forbidden) && return unless upload.downloadable?(current_user)

    path = upload.upload.path(params[:style])
    head(:bad_request) && return unless File.exist?(path) && params[:format].to_s == File.extname(path).gsub(/^\.+/, '')
    ...
    when :nginx then head(x_accel_redirect: path.gsub(Rails.root, ''), content_type: send_file_options[:type]) && return
    end
  ...
  def executable
    ...
    File.chmod(0o744, upload.upload.path)

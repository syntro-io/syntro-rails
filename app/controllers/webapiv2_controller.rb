...
class Webapiv2Controller < ApplicationController
  ...
  def _handle_custom_fields(request_data, item_type, item)
    ...
    if request_data['custom_fields'].is_a?(Array) \
      ...
      # symbolize keys
      request_data['custom_fields'].map!(&:symbolize_keys!)
  ...
  def _handle_test_case_tags(request_data, test_case)
    if request_data['tags'].is_a?(Array) \
      ...
      [false, 'tags was not an Array or is empty']
    end
  end
  ...
  def _item_statuses
    { 'response' => { 'message' => 'Retrieved statuses.',
  ...
  def _test_types(name = nil, id = nil)
    conditions = { name: name,
                   id: id }
    conditions.delete_if { |_k, v| v.blank? }
  ...
  def _get_category_hierarchy(category)
    ...
    [category_hierarchy, category_hierarchy_string]
  end
  ...
  def _users_roles(_request_data)
    { 'response' => { 'message' => (I18n.t :user_roles) },
      'status' => 200 }
  end
  ...
  def _devices_search(request_data)
    ...
    if devices != []
      if request_data['custom_fields']
        matching_devices =
          devices.map do |d|
            next unless _get_custom_fields('device', d) == request_data['custom_fields'].map(&:symbolize_keys!)
            { id: d[:id],
              name: d[:name],
  ...
  def _test_cases_handler(request_data, action)
    if request_data.key?('test_cases')
      if request_data['test_cases'].is_a?(Hash) \
        ...
        request_data['test_cases'].each do |_key, value|
          if action == 'create'
        ...
        { 'response' => { 'message' => 'Passed test_cases list was empty or invalid.' },
          'status' => 400 }
      end
    else
      if action == 'create'
        _test_cases_create(request_data)
      elsif action == 'update'
        _test_cases_update(request_data)
      end
    end
  end

  def _test_cases_create(request_data)
    request_data.delete('id') unless request_data['new_version']

    response_hash = _categories_search(request_data)
    if response_hash['status'] == 200 && !response_hash['response']['found']
      response_hash = _categories_create(request_data)
      return response_hash if response_hash['status'] != 201
    ...
    if response_hash['status'] == 200 \
      ...
      if response_hash['response']['found']
        ...
        if created_by_id
          ...
          unless response_hash['response']['found']
            response_hash['status'] = 400
            return response_hash
          end
        end
  ...
  def _test_cases_update(request_data)
    ...
    if response_hash['status'] == 200 && response_hash['response']['found']
      ...
      # handle tags
      test_case.tags.delete_all if request_data['new_values']['overwrite_tags']
  ...
  def _handle_plancases(test_plan, test_cases)
    if test_cases.is_a?(Array) \
      ...
      [false, nil, 'Passed test_cases parameter was empty or not an array.']
    end
  end

  def _test_plans_create(request_data)
    request_data['deprecated'] = 0 if request_data['new_version']
  ...
  def _handle_stencil_test_plans(stencil, test_plans, new_version = true)
    if test_plans.is_a?(Array) \
      ...
      unless stencil_test_plans.empty?
      ...
      test_plans.each_with_index do |tp, i|
        unless tp.key?('id') && tp.key?('device_id')
      ...
      [false, nil, 'Passed test_plans parameter was empty or not an array.']
    end
  end
  ...
  def _results_set(request_data)
    unless request_data.key?('results')
    ...
    if request_data['results'].is_a?(Hash) \
      ...
      request_data['results'].each do |key, value|
        ...
        unless bugs.nil?
          ...
          result.bugs = trimmed_bugs unless trimmed_bugs.empty?
        end
        ...
        unless previous_result.nil?
          ...
          unless bug_results.nil?
            open_bugs = []
            bug_results.sort.each do |key, value|
              issue_status = value[:status]
              # Redmine
              next unless Setting.value('Ticket System') == 'Redmine'
  ...
  def _attachments_handler(request_data, action)
    if request_data.key?('attachments')
      if request_data['attachments'].is_a?(Array) \
        ...
        { 'response' => { 'message' => 'Passed attachments list was empty or invalid.' },
          'status' => 400 }
      end
    else
      case action
      when 'upload'
        _attachments_upload(request_data)
      when 'update'
        _attachments_update(request_data)
      when 'delete'
        _attachments_delete(request_data)
      end
    end
  end
      ...
      response_hash['attachments'] =
        uploads.map do |u|
          { id: u[:id],
            ...
            updated_at: u.updated_at }.reject { |_k, v| v.nil? }
        end
  ...
  def _attachments_delete(request_data)
    ...
    attachment.destroy
    { 'response' => { 'message' => 'Successfully deleted attachment with id "%s".' % [request_data['id']] },
      'status' => 200 }
  end
end

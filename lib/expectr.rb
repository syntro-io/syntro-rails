...
class Expectr
  ...
  def initialize(cmd, args = {})
    ...
    raise ArgumentError, "String or File expected, was given #{cmd.class}" unless cmd.is_a?(String) || cmd.is_a?(File)
    cmd = cmd.path if cmd.is_a? File
    ...
    Thread.new do
      while @pid > 0
        next if select([@stdout], nil, nil, @timeout).nil?
        buf = ''
  ...
  def interact
    ...
    in_thread = Thread.new do
      input = ''
      send STDIN.getc.chr if select([STDIN], nil, nil, 1) while @pid > 0
    end
  ...
  def print_buffer(buf)
    ...
    if @logger_buffer =~ /\n$|\r$/
      ...
      LOGGER.info @logger_buffer.tr("\r", "\n").gsub(/\n\r$/, '').gsub(/\n$/, '')

namespace :install do
  ...
  task base: :environment do
    if User.count > 0 || Product.count > 0 || Version.count > 0
      puts 'Script could not be run.'
    else
      Setting.find_or_create_by(name: 'SystemID', value: (0...12).map { ('A'..'Z').to_a[rand(26)] }.join, description: 'System ID String.')
      Setting.find_or_create_by(name: 'Ticket System', value: 'none', description: 'The type of ticketing system to integrate with.')
      Setting.find_or_create_by(name: 'Ticket System Url', value: 'none', description: 'The url of the ticketing system.')
      Setting.find_or_create_by(name: 'Ticket System Username', value: 'none', description: 'The username for the the ticketing system user.')
      Setting.find_or_create_by(name: 'Ticket System Password', value: 'none', description: 'The password for the the ticketing system user.')
      Setting.find_or_create_by(name: 'Require Bug For Failed Result', value: 'Disabled', description: 'Require that a bug number is entered on a failed result.')
      Setting.find_or_create_by(name: 'Require Comment For Failed Result', value: 'Disabled', description: 'Require a comment for a failed result.')
      Setting.find_or_create_by(name: 'Require Bug For Blocked Result', value: 'Disabled', description: 'Require that a bug number is entered on a blocked result.')
      Setting.find_or_create_by(name: 'Require Comment For Blocked Result', value: 'Disabled', description: 'Require a comment for a blocked result.')
      Setting.find_or_create_by(name: 'Allow Test Case Edit After Execution', value: 'Enabled', description: 'Once a test case is executed, editing the test case is blocked if this is disabled. Versions can be used for changes if disabled.')
      Setting.find_or_create_by(name: 'Allow Test Plan Edit After Assignment', value: 'Enabled', description: 'Once a test plan is assigned, editing the test plan is blocked if this is disabled. Version can be used for changes if disabled.')
      Setting.find_or_create_by(name: 'Allow attachment execution', value: 'Disabled', description: 'To enable automation to run on the system (excluding the API), this feature must be enabled. Even with this enabled, only attachments that are marked as executable by an administrator or QA Manager will be executable.')
      Setting.find_or_create_by(name: 'jMeter Host', value: 'none', description: 'The hostname or IP address of the jMeter host system.')
      Setting.find_or_create_by(name: 'jMeter User', value: 'none', description: 'The username for the jMeter host.')
      Setting.find_or_create_by(name: 'jMeter Password', value: 'none', description: 'The password for the jMeter host. Note that this will not be used if a certificate is configured for login.')
      Setting.find_or_create_by(name: 'jMeter SSH Certificate', value: 'none', description: 'The certificate file that should be used to login to the jMeter system. This should be the full path to the file. Leave as none if you plan on using a password.')
      Setting.find_or_create_by(name: 'jMeter Working Directory', value: 'none', description: 'The jMeter working directory on the jMeter host. jMeter should save all logs to this folder. This is where TestCaseDB will run commands from. A full path should be provided. ex. /home/jmeter/working')
      Setting.find_or_create_by(name: 'jMeter Application Path', value: 'none', description: 'Location of the jMeter jar file on the jMeter host. Can be relative to the working directory (ex. apache-jmeter/bin/ApacheJMeter.jar) or a complete path (ex. /usr/local/apache-jmeter/bin/ApacheJMeter.jar)')
      Setting.find_or_create_by(name: 'jMeter Max Execution Time', value: '5', description: 'The maximum execution time for a jMeter test in minutes. If the test runs longer than this time it will be marked as failed.')
      Setting.find_or_create_by(name: 'Sikuli Host', value: 'none', description: 'The hostname or IP address of the Sikuli host system.')
      Setting.find_or_create_by(name: 'Sikuli User', value: 'none', description: 'The username for the Sikuli host.')
      Setting.find_or_create_by(name: 'Sikuli Password', value: 'none', description: 'The password for the Sikuli host. Note that this will not be used if a certificate is configured for login.')
      Setting.find_or_create_by(name: 'Sikuli SSH Certificate', value: 'none', description: 'The certificate file that should be used to login to the Sikuli system. This should be the full path to the file. Leave as none if you plan on using a password.')
      Setting.find_or_create_by(name: 'Sikuli Working Directory', value: 'none', description: 'The Sikuli working directory on the Sikuli host. Sikuli should save all logs to this folder. This is where TestCaseDB will run commands from. A full path should be provided. ex. /home/sikuli/working')
      Setting.find_or_create_by(name: 'Sikuli Application Path', value: 'none', description: 'Location of the Sikuli start script on the Sikuli host. Can be relative to the working directory (ex. Sikuli-IDE/sikuli-ide.sh) or a complete path (ex. /home/tcdb/Sikulix.y.z/Sikuli-IDE/sikuli-ide.sh)')
      Setting.find_or_create_by(name: 'Sikuli Max Execution Time', value: '5', description: 'The maximum execution time for a Sikuli test in minutes. If the test runs longer than this time it will be marked as failed.')
      ...
      TestType.find_or_create_by(name: 'Manual', description: 'For manually executed test cases.')
      TestType.find_or_create_by(name: 'Automated', description: 'For test cases run via automation.')
      TestType.find_or_create_by(name: 'jMeter', description: 'For test cases to be run using jMeter.')
      TestType.find_or_create_by(name: 'Sikuli', description: 'For test cases to be run using Sikuli.')
    ...
    products.each do |product|
      ...
      testcases.each do |tc|
        (1..10).each do |step_num|
          tc.steps.build(action: "This is step number #{step_num}.", result: "This is result  #{step_num}.", step_number: step_num)
        end
        tc.save
      end
      ...
      (1..10).each do |plan_num|
        tp = TestPlan.create(name: "#{product.name}: Test Plan #{plan_num}", description: "This is a sample test plan for #{product.name}.", product_id: product.id)
